...
class TestPatientDataInput:
    ...
    def test_surgeries_with_status(self):
        """Test Surgeries as list of {name, status: completed|scheduled}."""
        data = PatientDataInput(
            Age="45 years",
            Gender="Male",
            Diagnosis="Diabetes",
            Surgeries=[
                SurgeryWithStatus(name="Appendectomy 2010", status="completed"),
                SurgeryWithStatus(name="Knee replacement", status="scheduled"),
            ],
        )
        assert len(data.Surgeries) == 2
        assert data.Surgeries[0].name == "Appendectomy 2010"
        assert data.Surgeries[0].status == "completed"
        assert data.Surgeries[1].status == "scheduled"

    def test_surgeries_accepts_dicts_from_json(self):
        """Test Surgeries accepts list of dicts (e.g. from Postman/JSON body)."""
        data = PatientDataInput(
            Age="58 years",
            Gender="Male",
            Diagnosis="STEMI",
        ...
        assert data.Surgeries[0].name == "Appendectomy 2010"
        assert data.Surgeries[0].status == "completed"
        assert data.Surgeries[1].name == "Knee replacement"
        assert data.Surgeries[1].status == "scheduled"
