...
def scrub_phi(message: str) -> str:
    """Remove transcript/summary content from log messages."""
    scrubbed = message
...
class PHIFilter(logging.Filter):
    """Filter that redacts likely PHI from log records."""
...
def configure_logging() -> None:
    logging.basicConfig(
        ...
        format="%(asctime)s %(levelname)s %(name)s session=%(session_id)s event=%(event_type)s — %(message)s",
        force=True,
    )
...
def get_logger(name: str) -> logging.Logger:
    """Return a standard logger; pass session_id/event_type via extra=."""
