...
APP_DIR="/opt/clinical-summary-service"
...
cat > /etc/systemd/system/clinical-summary.service << EOF
[Unit]
...
ExecStart=$APP_DIR/venv/bin/uvicorn main:app --host 0.0.0.0 --port 8003
Restart=always
RestartSec=10
...
echo ""
echo "Check service status: systemctl status clinical-summary"
echo "View logs: journalctl -u clinical-summary -f"
